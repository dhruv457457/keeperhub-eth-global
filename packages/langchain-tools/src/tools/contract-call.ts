...
export function createContractCallTool(kh: KeeperHub): DynamicStructuredTool {
  return new DynamicStructuredTool({
    ...
    schema: z.object({
      ...
      contract: z.string().regex(/^0x[0-9a-fA-F]{40}$/).describe("Smart contract address (0x...)"),
      function: z.string().min(1).max(128).describe("Function name to call (e.g. 'balanceOf', 'transfer')"),
      args: z
        .array(z.unknown())
        .optional()
        .describe("Function arguments as a JSON array (e.g. ['0xABC...', '1000000'])"),
      abi: z.string().optional().describe("Contract ABI JSON string. Auto-fetched if omitted."),
      callType: z
        .enum(["read", "write"])
        .default("read")
        .describe("'read' for view/pure (no gas), 'write' for state-changing (costs gas)"),
      gasLimitMultiplier: z
        .string()
        .regex(/^\d+(\.\d+)?$/)
        .optional()
        .describe("Gas headroom multiplier e.g. '1.2' adds 20%. Write calls only."),
    }),
    func: async ({ network, contract, function: fn, args, abi, callType, gasLimitMultiplier }) => {
      try {
        if (callType === "read") {
          const result = await kh.web3.read({ network, contract, function: fn, args, abi });

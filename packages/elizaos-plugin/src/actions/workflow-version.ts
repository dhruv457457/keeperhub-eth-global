...
function extractTwoWorkflowIds(text: string): [string | null, string | null] {
  const all = [...text.matchAll(/\bwf_[a-zA-Z0-9_-]{1,64}\b/g)].map(
    (m) => m[0]
  );
...
export function createWorkflowVersionAction(kh: KeeperHub): Action {
  return {
    ...
    validate: async (
      ...
      const hasWf = /\bwf_[a-zA-Z0-9_-]{1,64}\b/.test(
    ...
    handler: async (
      ...
      if (!workflowId) {
        await callback?.({
          text: "❌ Please include a workflow ID (wf_xxx) to version.",
        });
        return false;
      }

      await callback?.({
        text: `📋 Creating new version of \`${workflowId}\`…`,
      });
    ...
    examples: [
      [
        {
          user: "{{user1}}",
          content: { text: "Create a v2 of workflow wf_abc123" },
        },
        {
          user: "{{agentName}}",
          content: {
            text: "📋 Creating new version of `wf_abc123`…",
            action: "KEEPERHUB_WORKFLOW_VERSION",
          },
        },
      ],
    ],
...
export function createWorkflowMigrateAction(kh: KeeperHub): Action {
  return {
    ...
    handler: async (
      ...
      if (!(fromId && toId)) {
        await callback?.({
          text: "❌ Please provide both workflow IDs: `migrate from wf_old to wf_new`",
        });
        return false;
      }
      ...
      try {
        // Drain old
        const drainObs = await kh.tryRun(fromId, {
          input: { _action: "withdraw" },
          wait: true,
        });
        const drainMsg = drainObs.ok
          ? "✅ Old workflow drained."
          : `⚠️ Drain: ${drainObs.summary}`;

        // Activate new
        const activateObs = await kh.tryRun(toId, { input: {}, wait: true });
        const activateMsg = activateObs.ok
          ? "✅ New workflow activated."
          : `❌ Activation: ${activateObs.summary}`;
    ...
    examples: [
      [
        {
          user: "{{user1}}",
          content: { text: "Migrate from wf_old123 to wf_new456" },
        },
        {
          user: "{{agentName}}",
          content: {
            text: "🔄 Migrating from `wf_old123` → `wf_new456`…",
            action: "KEEPERHUB_WORKFLOW_MIGRATE",
          },
        },
      ],
    ],

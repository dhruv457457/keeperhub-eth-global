...
function detectMathOperation(text: string): string | null {
  ...
  if (lower.includes("average") || lower.includes("avg") || lower.includes("mean")) return "average";
  if (lower.includes("max") || lower.includes("maximum") || lower.includes("largest")) return "max";
  if (lower.includes("min") || lower.includes("minimum") || lower.includes("smallest")) return "min";
...
export function createRunCodeAction(kh: KeeperHub): Action {
  return {
    ...
    similes: [
      "EXECUTE_CODE", "RUN_JAVASCRIPT", "RUN_JS", "CODE_EXECUTE",
      "MATH_AGGREGATE", "AGGREGATE_VALUES", "CALCULATE",
      "SUM_VALUES", "AVERAGE_VALUES",
    ],
    description:
    ...
    validate: async (_runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
      ...
      const hasMath = /\b(sum|average|avg|median|max|min|product|aggregate|calculate)\b/i.test(text)
        && /\d+(?:\.\d+)?/.test(text);
    ...
    handler: async (
      ...
      if (mathOp && !hasCode) {
        ...
        switch (mathOp) {
          ...
          case "average": result = values.reduce((a, b) => a + b, 0) / n; break;
          case "max":     result = Math.max(...values); break;
          case "min":     result = Math.min(...values); break;
          case "median":  result = n % 2 === 0 ? (sorted[n / 2 - 1] + sorted[n / 2]) / 2 : sorted[Math.floor(n / 2)]; break;
      ...
      await callback?.({ text: "⚡ Executing code via KeeperHub sandboxed VM…" });
      ...
      try {
        ...
        if (!obs.ok) {
          elizaLogger.error(`[KeeperHub] Code execution failed: ${obs.error?.message}`);
          await callback?.({ text: `❌ Code execution failed: ${obs.error?.message}` });
          return false;
        }
        ...
        await callback?.({ text: `❌ Error: ${err instanceof Error ? err.message : String(err)}` });
        return false;
      }
    },
    ...
    examples: [
      [
        { user: "{{user1}}", content: { text: "Run this:\n```js\nreturn { doubled: inputs.amount * 2 }\n```" } },
        { user: "{{agentName}}", content: { text: "⚡ Executing code via KeeperHub sandboxed VM…", action: "KEEPERHUB_RUN_CODE" } },
      ],
      [
        { user: "{{user1}}", content: { text: "Average these APY rates: 3.2, 4.1, 5.6, 2.8" } },
        { user: "{{agentName}}", content: { text: "🔢 **average**(3.2, 4.1, 5.6, 2.8) = **3.925**", action: "KEEPERHUB_RUN_CODE" } },
      ],
    ],
  };
}

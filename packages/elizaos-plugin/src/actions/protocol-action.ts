 ...
 */
function parseProtocolSpec(text: string): { actionType: string; params: Record<string, unknown> } | null {
  ...
  try {
    ...
    if (parsed["actionType"] && parsed["params"]) {
      return { actionType: String(parsed["actionType"]), params: parsed["params"] as Record<string, unknown> };
    }
...
const PROTOCOL_NAMES: Record<string, string> = {
  "aave-v3": "Aave V3", "aave-v4": "Aave V4", "uniswap": "Uniswap",
  "lido": "Lido", "compound-v3": "Compound V3", "curve": "Curve",
  "morpho": "Morpho", "yearn-v3": "Yearn V3", "aerodrome": "Aerodrome",
  "cowswap": "CowSwap", "rocket-pool": "Rocket Pool", "pendle": "Pendle",
  "sky": "Sky", "spark": "Spark", "ethena": "Ethena", "safe": "Safe",
};
...
export function createProtocolActionElizaAction(kh: KeeperHub): Action {
  return {
    ...
    similes: [
      "DEFI_ACTION", "AAVE_ACTION", "UNISWAP_ACTION", "LIDO_ACTION",
      "COMPOUND_ACTION", "CURVE_ACTION", "PROTOCOL_EXECUTE",
      "SUPPLY_AAVE", "SWAP_UNISWAP", "STAKE_LIDO",
    ],
    description:
    ...
    validate: async (_runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
      ...
      const hasProtocol = /aave|uniswap|lido|compound|curve|morpho|yearn|aerodrome|cowswap|rocket.pool|pendle|spark|ethena/i.test(text);
      const hasAction = /supply|borrow|withdraw|repay|swap|stake|wrap|deposit|exchange|harvest/i.test(text);
    ...
    handler: async (
      ...
      try {
        ...
        if (executionId) {
          ...
          lines.push(`💡 Use \`check execution ${executionId}\` to monitor progress.`);
    ...
    examples: [
      [
        ...
        {
          user: "{{agentName}}",
          content: { text: "⚡ Executing **Aave V3** `supply` action via KeeperHub…", action: "KEEPERHUB_PROTOCOL_ACTION" },
        },
      ],
    ],
  };

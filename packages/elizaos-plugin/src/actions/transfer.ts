...
function extractTokenAddress(
  ...
  const allAddresses = [...text.matchAll(/\b(0x[0-9a-fA-F]{40})\b/g)].map(
    (m) => m[1]
  );
  const others = allAddresses.filter(
...
export function createTransferAction(kh: KeeperHub, opts: TransferActionOptions = {}): Action {
  return {
    ...
    handler: async (
      ...
      if (!recipient) {
        await callback?.({
          text: "❌ I couldn't find a recipient address. Please include a 0x address.",
        });
        return false;
      }
      ...
      if (!amountInfo) {
        await callback?.({
          text: "❌ I couldn't parse the amount. Please specify e.g. '0.01 ETH' or '100 USDC'.",
        });
        return false;
      }
      ...
      [
        {
          user: "{{user1}}",
          content: {
            text: "Send 0.01 ETH to 0xAbCd1234567890AbCd1234567890AbCd12345678 on Base",
          },
        },
        {
          user: "{{agentName}}",
